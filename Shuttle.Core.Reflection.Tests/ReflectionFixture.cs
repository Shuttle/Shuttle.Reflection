...
public class ReflectionFixture
{
    ...
    {
        Assert.That((await Assembly.GetTypesCastableToAsync<SomeClass>()).Count(), Is.EqualTo(1));
        Assert.That((await Assembly.GetTypesCastableToAsync<ISomeClass>()).Count(), Is.EqualTo(1));
    }
}